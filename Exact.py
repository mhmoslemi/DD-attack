...
Three crafting objectives, selected by --attack:
  ...
  influence : directly drive the inverse-Hessian target-loss influence as negative
              as possible.

Surrogates (the frozen feature extractors used by selection and crafting) share
the VICTIM architecture (--model) and are trained from scratch on the full real
training set. The victim is likewise trained from scratch on the full poisoned
50k set with the plain MetaPoison schedule (no augmentation, no weight decay).

Pipeline:
  S0. train K surrogates on the full real train set.
  ...
  per (class_pair, target):
    ...
    2. craft L_inf<=eps poisons (fc / gradmatch / influence).
...
Example (gradient matching):
  python eval_standard_nodistill.py \
      --model ConvNetBN --class_pairs dog-bird \
...
Example (theory-consistent influence-based base selection):
  python eval_standard_nodistill.py \
      --model ConvNetBN --class_pairs dog-bird \
      --attack gradmatch --exact_select
"""
...
# --------------------------------------------------------------------------- #
# from-scratch trainer (MetaPoison schedule: no aug, no weight decay). Used for
# both surrogates and victims.
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# surrogate ensemble trained on the full real training set
# --------------------------------------------------------------------------- #
def train_surrogates_on_full(train_imgs, train_labs, test_imgs, test_labs,
                             channel, num_classes, im_size, args, device):
    """Train surrogates on the full real training set (SGD like victims)."""
...
# --------------------------------------------------------------------------- #
# INFLUENCE-FUNCTION base selection.
#
# score='mixed'  (paper S_base, Sec 3.2 -- THEORY-CONSISTENT, default):
#     S_base(z, z_t) = || [grad_x grad_th l(z, th*)]^T (H+lambda I)^-1 grad_th l(z_t) ||_1
#                    = || C_x^T v_t ||_1,     v_t = (H + lambda I)^-1 grad_th l(z_t).
#   After the first-order expansion of the poison gradient around the clean base,
#   the base gradient cancels and only the perturbation contribution survives;
#   eps*||C_x^T v_t||_1 is exactly the best target-loss influence achievable by an
#   L_inf-eps perturbation (delta* = eps*sign(C_x^T v_t)). We rank base points by
#   this best-case poison. C_x^T v_t is obtained by
#     mixed_method='exact': grad_x <grad_th l(z), v_t> via ONE double-backward per
#                           candidate batch (summed loss so cross terms vanish),
#                           no finite-difference error; or
#     mixed_method='fd'   : ||v_t|| * ( grad_x l(x, th*+h v_hat)
#                                     - grad_x l(x, th*-h v_hat) ) / (2h),
#                           two batched input-gradient passes, NO second-order graph
#                           (cheaper; O(h^2) FD error, uses --if_fd_h).
#
# score='cleanpoint'  (legacy Koh-Liang influence of the CLEAN base point):
#     score(z) = grad_th l(z_t)^T (H+lambda I)^-1 grad_th l(z).
#   Ignores the perturbation direction entirely -- this is NOT the paper's S_base.
#   Kept only to reproduce older runs.
#
# (H + lambda I)^-1 g_t is solved matrix-free by conjugate gradients over
# Hessian-vector products (never forming H). H is the surrogate's empirical-risk
# Hessian on the Hessian set. Per-surrogate scores are standardized before being
# summed over the ensemble (a monotone affine map: this preserves the ranking for
# a single surrogate and stops any one surrogate's magnitude/conditioning from
# dominating when averaging several). Largest score = best base to poison.
# --------------------------------------------------------------------------- #
...
def _per_sample_input_grad(net, imgs, y_const, device, bs=256):
    """Per-sample input gradient  grad_x CE(net(x), y_const)  at the CURRENT params.
    The batch loss is SUMMED, so a single backward isolates each sample's own
    input gradient (each sample's logits depend only on its own row, so the
    cross terms vanish in eval mode). Returns a tensor shaped like `imgs`."""
    net.eval()
    outs = []
    for i in range(0, len(imgs), bs):
        b = imgs[i:i + bs].detach().clone().requires_grad_(True)
        yb = torch.full((len(b),), y_const, dtype=torch.long, device=device)
        loss = F.cross_entropy(net(b), yb, reduction='sum')
        gx = torch.autograd.grad(loss, b)[0]
        outs.append(gx.detach())
...
def select_base_influence(surrogates, images_norm, labels, x_t_norm, y_adv, N_p,
                          ...
                          last_layer=False, hess_bs=0, max_surrogates=0,
                          score='mixed', mixed_method='exact', cand_bs=128,
                          verbose=False):
    """Rank class-y_adv candidates by an influence-function base-selection score,
    averaged over the surrogate ensemble. See the block comment above for the two
    scores ('mixed' = paper S_base, default; 'cleanpoint' = legacy). Returns
    indices into the full training set (largest score = best base to poison)."""
    cls_idx = (labels == y_adv).nonzero(as_tuple=True)[0]
    if len(cls_idx) < N_p:
        raise ValueError('class %d has %d images < N_p=%d' % (y_adv, len(cls_idx), N_p))
    cand = images_norm[cls_idx]
    ...
    for net in nets:
        ...
        # v_t = (H + damping I)^-1 grad_th l(z_t)   (z_t label = y_adv; CG over HVPs)
        y_t = torch.tensor([y_adv], dtype=torch.long, device=device)
        loss_t = F.cross_entropy(net(x_t_norm.unsqueeze(0)), y_t)
        g_t = [gi.detach() for gi in torch.autograd.grad(loss_t, params)]
        v_t, used = _cg_inverse_hvp(net, params, g_t, hess_imgs, hess_labs, device,
                                    damping, cg_iters, cg_tol, hess_bs)
        cg_used.append(used)

        sur_score = torch.zeros(len(cls_idx), device=device)

        if score == 'cleanpoint':
            # legacy scalar score  <v_t, grad_th l(z)>  via central FD over params
            snorm = torch.sqrt(sum((si * si).sum() for si in v_t)).clamp_min(1e-12)
            shat = [si / snorm for si in v_t]
            ...
            with torch.no_grad():
                ...
                for p, o in zip(params, orig):
                    p.copy_(o)
            sur_score = snorm * (lp - lm) / (2.0 * fd_h)

        elif mixed_method == 'fd':
            # S_base via FD-over-params:  C_x^T v_t = grad_x <grad_th l(z), v_t>
            #   ~ ||v_t|| * (grad_x l(x, th*+h vhat) - grad_x l(x, th*-h vhat)) / 2h
            snorm = torch.sqrt(sum((si * si).sum() for si in v_t)).clamp_min(1e-12)
            shat = [si / snorm for si in v_t]
            orig = [p.detach().clone() for p in params]
            with torch.no_grad():
                for p, o, sh in zip(params, orig, shat):
                    p.copy_(o + fd_h * sh)
            gx_p = _per_sample_input_grad(net, cand, y_adv, device, cand_bs)
            with torch.no_grad():
                for p, o, sh in zip(params, orig, shat):
                    p.copy_(o - fd_h * sh)
            gx_m = _per_sample_input_grad(net, cand, y_adv, device, cand_bs)
            with torch.no_grad():
                for p, o in zip(params, orig):
                    p.copy_(o)
            cx_v = snorm * (gx_p - gx_m) / (2.0 * fd_h)          # (N, C, H, W)
            sur_score = cx_v.abs().flatten(1).sum(1)

        else:  # mixed_method == 'exact' : double-backward, no FD error on inputs
            for st in range(0, len(cand), cand_bs):
                xb = cand[st:st + cand_bs].detach().clone().requires_grad_(True)
                yb = torch.full((len(xb),), y_adv, dtype=torch.long, device=device)
                loss_b = F.cross_entropy(net(xb), yb, reduction='sum')
                g_b = torch.autograd.grad(loss_b, params, create_graph=True)
                dot = sum((gb * si).sum() for gb, si in zip(g_b, v_t))
                cx_v = torch.autograd.grad(dot, xb)[0]           # (b, C, H, W)
                sur_score[st:st + len(xb)] = cx_v.abs().flatten(1).sum(1)

        # standardize per surrogate before accumulating (monotone for 1 surrogate,
        # prevents any single surrogate dominating the ensemble average)
        total += standardize(sur_score.detach())
    ...
    total /= len(nets)
    if verbose:
        print('      [IF score=%s method=%s] CG iters/surrogate: %s'
              % (score, mixed_method if score == 'mixed' else '-', cg_used))
...
def _poison_key(args, pair, tidx):
    """Filename for caching a crafted poison so runs can re-use it. The selection
    method is encoded explicitly so poisons crafted with different base selectors
    NEVER collide in the cache."""
    if args.random_select:
        sel = 'rand'
    elif args.exact_select:
        sel = ('if-cleanpoint' if args.if_score == 'cleanpoint'
               else 'if-mixed-%s' % args.if_mixed_method)
    elif args.curv_select:
        sel = 'curv'
    else:
        sel = 'ml' if args.multilayer else 'sl'
    ss = 'ss' if args.single_surrogate else 'ens'
    return ('poison_%s_%s_%s_b%d_eps%d_st%d_ns%d_%s_%s_%s_t%d_seed%d.pt' % (
        args.attack, args.model, pair, round(args.budget * 1e4),
...
def _victim_worker(phys_gpu, gpu_rank, victim_idxs, model_name, channel,
    ...
    for vi in victim_idxs:
        ...
        net = train_from_scratch(net, poisoned, labels, args.victim_epochs,
...
def train_victims_parallel(gpu_pool, num_victims, model_name, channel, num_classes,
                           ...
                           test_imgs_cpu, test_labs_cpu, args):
    ...
    ret_q, procs = _start_workers(
        ...
        (model_name, channel, num_classes, im_size, poisoned_cpu, labels_cpu,
         x_t_cpu, test_imgs_cpu, test_labs_cpu, args))
...
def train_surrogates_on_full_parallel(gpu_pool, train_imgs_cpu, train_labs_cpu,
    ...
    ret_q, procs = _start_workers(
        ...
        (args.model, channel, num_classes, im_size, train_imgs_cpu,
    ...
    return _rebuild_surrogates(results, args.model, channel, num_classes,
...
def main(args):
    ...
    # is pinned to its first GPU so its own work (crafting, selection) stays
    # single-GPU and get_network() never wraps in DataParallel.
    ...
    # ---- train surrogates on the full real data ---------------------------
    sur_cache = os.path.join(args.cache_dir,
        'surrogates_%s_fulldata_%dx%dep_seed%d' % (
            args.model, args.num_surrogates, args.surrogate_epochs, args.seed)
    ...
    if sur_cache and all(
        ...
        print('\n%s === training %d surrogates (%s) on FULL real data (%d ep each)%s ==='
              % (get_time(), args.num_surrogates, args.model, args.surrogate_epochs,
    ...
    if args.clean_baseline:
        vic_cache = os.path.join(args.cache_dir,
            'clean_victims_%s_%dx%dep_seed%d' % (
                args.model, n_clean, args.victim_epochs, args.seed)
        ) if args.cache_dir else ''
        if vic_cache and all(
                os.path.exists(os.path.join(vic_cache, 'victim_%d.pt' % i))
                for i in range(n_clean)):
            print('\n%s === loading %d clean victim(s) (%s) from cache: %s ==='
                  % (get_time(), n_clean, args.model, vic_cache))
            for i in range(n_clean):
                net = get_network(args.model, channel, num_classes, im_size)
                net.load_state_dict(torch.load(
                    os.path.join(vic_cache, 'victim_%d.pt' % i), map_location=device))
                clean_victims.append(net.to(device).eval())
        else:
            print('\n%s === training %d clean victim(s) (%s) from scratch on clean data ==='
                  % (get_time(), n_clean, args.model))
            for i in range(n_clean):
                net = get_network(args.model, channel, num_classes, im_size)
        ...
        clean_cta = float(np.mean(
            [test_acc(n, test_imgs, test_labs, device) for n in clean_victims]))
    ...
    # ---- Hessian set for INFLUENCE-FUNCTION selection ----------------------
    # = a random subsample of the real training set (the data the surrogate's
    # empirical risk is defined on). Built once.
    ...
    if args.exact_select:
        ...
        print('%s IF Hessian set = %d real train images' % (get_time(), len(hess_imgs)))
    ...
    for pair in args.class_pairs:
        ...
        if pair in preselected:
            ...
            # drifts run-to-run. Cache the chosen indices per (model, pair,
            # num_targets, seed, easiness) so repeat runs attack the SAME targets.
            tcache = (os.path.join(args.target_cache_dir,
                      'easy_targets_%s_%s_n%d_seed%d_e%.2f.json'
                      % (args.model, pair, args.num_targets, args.seed,
                         args.easy_targets))
            ...
            if tcache and os.path.exists(tcache):
                ...
                if tcache:
                    ...
                    with open(tcache, 'w') as _f:
                        json.dump({'model': args.model, 'pair': pair,
        ...
        # accumulators over all target x victim votes
        stat = {'asr_each': [], 'cta_each': [], 'clean': [],
        ...
        for ti, tidx in enumerate(chosen):
            ...
            if pkey and os.path.exists(pkey):
                ...
                if args.random_select:
                    ...
                    base_idx = select_base_influence(
                        ...
                        max_surrogates=args.if_max_surrogates,
                        score=args.if_score, mixed_method=args.if_mixed_method,
                        cand_bs=args.if_cand_bs, verbose=args.verbose)
                else:
            ...
            # method clean ASR for this target
            if args.clean_baseline:
                clean_asr = 100.0 * sum(predict_target(n, x_t_norm) == y_adv
                                        for n in clean_victims) / len(clean_victims)
            else:
                clean_asr = float('nan')

            # 4) train victims, measure ASR/CTA
            victim_preds, victim_ctas = [], []
            if use_parallel:
                print('    victims: %d across GPUs %s ... '
                      % (args.num_victims, gpu_pool), end='', flush=True)
                ...
                for vi in range(args.num_victims):
                    ...
                    net = train_from_scratch(net, poisoned, train_labs, args.victim_epochs,
                                             args.victim_lr, args.victim_bs,
                                             args.victim_decay, device)
            ...
            stat['asr_each'].extend(100.0 if p == y_adv else 0.0 for p in victim_preds)
            stat['cta_each'].extend(float(c) for c in victim_ctas)
            stat['clean'].append(clean_asr)

            print('    [%s t%d/%d idx=%d] poison_CTA=%.4f poison_ASR=%.0f%%%s'
                  % (pair, ti + 1, len(chosen), tidx, poison_cta, poison_asr,
                     ('  clean_CTA=%.4f clean_ASR=%.0f%%' % (clean_cta, clean_asr))
                     if args.clean_baseline else ''))

            all_rows.append({
                'pair': pair, 'attack': args.attack,
                'y_adv': y_adv, 'target_class': target_class, 'target_idx': tidx,
                'target_true_label': int(test_labs[tidx]),
                'clean_cta': clean_cta, 'clean_asr': clean_asr,
        ...
        pa, ct = np.array(stat['asr_each']), np.array(stat['cta_each'])
        line = ('    poison CTA = %.4f +/- %.4f   poison ASR = %.1f%% +/- %.1f%%'
                % (ct.mean(), ct.std(), pa.mean(), pa.std()))
        if args.clean_baseline:
            line += ('   | clean CTA = %.4f   clean ASR = %.1f%%'
                     % (clean_cta, float(np.nanmean(stat['clean']))))
        print(line)
        print('      tally (%s): %s' % (class_names, stat['tally'].tolist()))
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(
        description='Standard from-scratch (no victim distillation) eval on full '
                    'real data; surrogates share the victim arch (--model); '
                    '--attack fc | gradmatch | influence.')
    # data / model
    ...
    p.add_argument('--model', type=str, default='ConvNetBN',
                   help="VICTIM arch, also used for the surrogates (ConvNetBN to "
                        "match MetaPoison; this repo's ConvNetBN is depth-3, not the "
                        "6-layer Finn net)")
    ...
    p.add_argument('--dsa_strategy', type=str,
                   ...
                   help='DiffAugment strategy used inside gradmatch crafting')
    # surrogates (selection + crafting feature extractors), trained on full real data
    ...
    p.add_argument('--target_cache_dir', type=str, default='',
                   help='dir to save/load the --easy_targets selection (keyed by '
                        'model/pair/num_targets/seed). First run saves the chosen '
                        'target indices; later runs reuse them verbatim so the attack '
                        'hits the SAME targets despite surrogate re-training.')
    ...
    p.add_argument('--num_clean_victims', type=int, default=None,
                   help='# clean victims for the baseline (default: num_victims). '
                        '1 is enough to read the clean-accuracy ceiling.')
    ...
    # ------------------------------------------------------------------- #
    # INFLUENCE-FUNCTION base selection (--exact_select). The paper's S_base
    # (Sec 3.2) is the DEFAULT scoring here; the knobs below are pre-set to the
    # values used in the reference run, so a run needs only `--exact_select`.
    # ------------------------------------------------------------------- #
    p.add_argument('--exact_select', action='store_true', default=False,
                   help='use influence-function base selection instead of the scored/'
                        'curv/random selectors. Default score is the paper S_base '
                        '||C_x^T (H+lambda I)^-1 g_t||_1 (see --if_score). Mutually '
                        'exclusive with --random_select. Selection only; crafting is '
                        'still --attack (fc/gradmatch).')
    p.add_argument('--if_score', type=str, default='mixed',
                   choices=['mixed', 'cleanpoint'],
                   help="THEORY-CONSISTENT default 'mixed' = paper S_base "
                        "||C_x^T (H+lambda I)^-1 g_t||_1 (best-case L_inf-eps poison "
                        "influence). 'cleanpoint' = legacy clean-point influence "
                        "g_t^T H^-1 grad l(z) (ignores the perturbation; reproduces "
                        "older runs).")
    p.add_argument('--if_mixed_method', type=str, default='exact',
                   choices=['exact', 'fd'],
                   help="for --if_score mixed: 'exact' = double-backward "
                        "grad_x<grad_th l(z), v_t> (no finite-diff error, default); "
                        "'fd' = central finite difference over params (cheaper, uses "
                        "--if_fd_h, O(h^2) error).")
    p.add_argument('--if_cand_bs', type=int, default=128,
                   help='candidate batch size for the mixed-derivative pass (lower if it '
                        'OOMs; per-sample scores are batch-size invariant)')
    ...
    p.add_argument('--if_fd_h', type=float, default=1e-2,
                   help='central finite-difference step over params; only used for '
                        '--if_score cleanpoint or --if_mixed_method fd (unused for the '
                        'default exact path)')
    ...
    p.add_argument('--if_hess_size', type=int, default=2000,
                   help='subsample size of the real-data Hessian set (default 2000; '
                        '0 also falls back to 2000)')
    p.add_argument('--if_max_surrogates', type=int, default=1,
                   help='cap how many surrogates the IF score averages over (default 1; '
                        'set 0 to use all surrogates)')
    p.add_argument('--verbose', action='store_true', default=False,
                   help='extra logging (e.g. influence-selection conjugate-gradient iters used)')
    ...
    p.add_argument('--poison_cache_dir', type=str, default='',
                   help='dir to save/load crafted poisons (base_idx + perturbed image) '
                        'so runs can be repeated without re-selecting/re-crafting')
